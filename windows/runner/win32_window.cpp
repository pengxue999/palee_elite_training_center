...
Win32Window::MessageHandler(HWND hwnd,
  ...
  switch (message) {
    ...
    case WM_GETMINMAXINFO: {
      ...
      // Set minimum client area to 400x400, then calculate total window size including borders
      RECT clientRect = {0, 0, 400, 400};
      DWORD style = GetWindowLong(window_handle_, GWL_STYLE);
      DWORD exStyle = GetWindowLong(window_handle_, GWL_EXSTYLE);
      BOOL hasMenu = (GetMenu(window_handle_) != nullptr);
